...
import { useStatisticsByProcess } from '../../hooks/useStatistics'
...
const ProcessNgRateChart = ({ startDate, endDate }) => {
  ...
  const { data: statistics, isLoading, error } = useStatisticsByProcess(params)
  ...
  const chartData = useMemo(() => {
    ...
    const labels = statistics.map((stat) => `${stat.processCode} - ${stat.processName}`)
    const ngRates = statistics.map((stat) => parseFloat(stat.ngRate || 0))
      ...
      tooltip: {
        callbacks: {
          label: function (context) {
            return `NG 비율: ${context.parsed.y.toFixed(2)}%`
          }
        }
      }
    },
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
      <h3 className="text-sm font-medium text-gray-500 mb-4">공정별 NG 비율</h3>
      <div className="h-80">
...
export default ProcessNgRateChart
